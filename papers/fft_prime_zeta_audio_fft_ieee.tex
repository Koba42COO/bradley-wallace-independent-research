...
\subsection{Research Timeline (Feb--Nov 2024)}

The work reported here grew out of a continuous nine-month exploration that can be summarized in four phases:
\begin{enumerate}
  \item \textbf{Phase I (Feb--Apr 2024):} Initial experiments with Wallace Transform parameterizations, small-scale prime and zeta visualizations, and early audio prototypes using simple sine synthesis.
  \item \textbf{Phase II (May--Jul 2024):} Integration of Bram Cohen's \texttt{xen12synth}, construction of stable prime and zeta audio mappings, and the first contrary-motion experiments culminating in the \texttt{prime\_zeta\_contrary\_motion.py} pipeline.
  \item \textbf{Phase III (Aug--Sep 2024):} Development of the FFT RTA analyzer, systematic generation of spectra and spectrograms for all audio tracks, and consolidation of results into JSON logs and PNG figures.
  \item \textbf{Phase IV (Oct--Nov 2024):} Integration with UPG and AIVA infrastructure, critical review of earlier prime-prediction claims, and the shift from exploratory narratives to the reproducible, code-grounded formulation presented in this paper.
\end{enumerate}
...
\subsection{FFT Visualizations}

When \texttt{fft\_rta\_harmonic\_analyzer.py} is run after the audio generation step, it produces one RTA-style PNG figure per WAV file.
These can be included directly into this paper as follows:

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{prime_distribution_track_rta_analysis.png}
  \caption{RTA harmonic analysis of the prime distribution audio track. Top-left: FFT magnitude spectrum with peak markers; top-right: spectrogram; bottom-left: harmonic magnitudes; bottom-right: spectral centroid and roll-off.}
  \label{fig:prime-rta}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{zeta_distribution_track_rta_analysis.png}
  \caption{RTA harmonic analysis of the zeta distribution audio track, constructed from truncated $\widehat{\zeta}(k)$ values mapped to pitch.}
  \label{fig:zeta-rta}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{prime_zeta_contrary_motion_rta_analysis.png}
  \caption{RTA harmonic analysis of the mixed prime--zeta contrary-motion track, showing the combined spectral content of both voices.}
  \label{fig:mixed-rta}
\end{figure}

The filenames in Figures~\ref{fig:prime-rta}--\ref{fig:mixed-rta} match exactly the output naming convention in the analyzer: for an input \texttt{X.wav}, the figure file is \texttt{X\_rta\_analysis.png}.
If the user additionally runs the analyzer on \texttt{207\_zeta\_tritone\_kintu.wav}, a corresponding \texttt{207\_zeta\_tritone\_kintu\_rta\_analysis.png} figure will be generated and can be included analogously.
...
The author thanks Julieanna and the VANTA X team for early testing and feedback on the PAC/tri-kernel infrastructure and for helping to shape and challenge the unified Wallace Transform framework during its initial development.
