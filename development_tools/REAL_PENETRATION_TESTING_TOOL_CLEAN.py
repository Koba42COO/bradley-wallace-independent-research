...
"""
        ...
        dns_recon  results['dns_reconnaissance']
        summary  f"Target: {dns_recon.target}n"
        summary  f"IP Addresses: {', '.join(dns_recon.ip_addresses) if dns_recon.ip_addresses else 'None found'}n"
        summary  f"Subdomains: {', '.join(dns_recon.subdomains) if dns_recon.subdomains else 'None found'}nn"
        
        summary  "DNS Records:n"
...
def main():
    ...
    try:
        ...
        print(f"nSECURITY ASSESSMENT COMPLETED!")
