...
class ClaudeDataImporter:
    ...
    def __init__(self, dump_path: str, output_path: str = "./claude_imported_data"):
        ...
        # Options
        self.corpus_dir: Path = self.output_path / "corpus"
        self.max_corpus_chunk_chars: int = 1200
    ...
    def _chunk_text(self, text: str, max_chars: int = 1200) -> List[str]:
        """Chunk text for retrieval: prefer paragraph breaks, then sentence-ish splits."""
        if not text:
            return []
        ...
        for p in paragraphs:
            if len(p) <= max_chars:
                ...
                # Rough sentence splitter; keeps punctuation
    ...
    def export_corpus(self) -> Dict[str, Any]:
        """Export a retrieval-friendly plain-text corpus from projects and conversations.
        Writes .txt files into <output>/corpus with simple metadata and returns stats.
        """
        logger.info("Exporting retrieval corpus...")
        ...
        for proj in self.projects:
            base = f"project_{proj.uuid[:8]}"
            chunks = self._chunk_text(proj.content, self.max_corpus_chunk_chars)
            for idx, ch in enumerate(chunks, start=1):
                fn = self.corpus_dir / f"{base}_chunk{idx:03d}.txt"
        ...
        # Conversations (flatten messages)
        for conv in self.conversations:
            for i, msg in enumerate(conv.messages, start=1):
                text_parts = [msg.text or ""]
                # include any textual content fields inside content[]
                for c in msg.content:
                    if isinstance(c, dict) and 'text' in c and isinstance(c['text'], str):
                        text_parts.append(c['text'])
                combined = "\n\n".join([t for t in text_parts if t])
                for idx, ch in enumerate(self._chunk_text(combined, self.max_corpus_chunk_chars), start=1):
                    fn = self.corpus_dir / f"conv_{conv.uuid[:8]}_msg{i:03d}_chunk{idx:03d}.txt"
                    with open(fn, 'w', encoding='utf-8') as f:
                        f.write(ch)
                    files_written += 1
                    meta_lines.append(
        ...
        # Write manifest
        manifest = self.corpus_dir / "manifest.jsonl"
        with open(manifest, 'w', encoding='utf-8') as f:
            for line in meta_lines:
                f.write(line + "\n")

        logger.info(f"Corpus export complete: {files_written} chunks -> {self.corpus_dir}")
        return {'chunks_written': files_written, 'manifest': str(manifest)}
...
'''
    ...
    def _generate_import_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'corpus_path': str(self.corpus_dir) if self.corpus_dir.exists() else None
        }
...
def main():
    """Main import function with CLI options"""
    parser = argparse.ArgumentParser(description="Import Claude data dump and build project structure")
    parser.add_argument("dump_path", help="Path to Claude data dump (folder containing users.json, projects.json, conversations.json)")
    parser.add_argument("--output", "-o", default="./claude_imported_data", help="Output directory (default: ./claude_imported_data)")
    parser.add_argument("--export-corpus", action="store_true", help="Also export a retrieval corpus of chunked .txt files")
    parser.add_argument("--max-chars", type=int, default=1200, help="Max characters per corpus chunk (default 1200)")

    args = parser.parse_args()

    if not os.path.exists(args.dump_path):
        print(f"Error: Path '{args.dump_path}' does not exist")
        sys.exit(1)

    importer = ClaudeDataImporter(args.dump_path, args.output)
    importer.max_corpus_chunk_chars = max(200, args.max_chars)
    summary = importer.import_all_data()

    if args.export_corpus:
        corpus_stats = importer.export_corpus()
    ...
    categories = summary.get('project_categories', {})
    langs = summary.get('language_breakdown', {})
    print(f"\nProject Categories: {', '.join(categories.keys())}")
    if langs:
        top_langs = sorted(langs.items(), key=lambda x: x[1], reverse=True)
        top_langs_str = ', '.join([f"{k}:{v}" for k,v in top_langs[:5]])
        print(f"Top Languages: {top_langs_str}")

    if args.export_corpus:
        print(f"\nCorpus: wrote {summary['corpus']['chunks_written']} chunks -> {importer.corpus_dir}")
        print(f"Manifest: {summary['corpus']['manifest']}")
