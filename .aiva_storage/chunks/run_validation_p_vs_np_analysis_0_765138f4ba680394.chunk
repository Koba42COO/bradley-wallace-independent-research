#!/usr/bin/env python3
"""
Validation script for p_vs_np_analysis
...
def run_validation():
    ...
    test_file = tests_dir / f"test_p_vs_np_analysis.py"
    if test_file.exists():
    ...
    # Save results
    results_file = output_dir / f"validation_results_p_vs_np_analysis.json"
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Generate markdown report
    report_file = output_dir / f"validation_log_p_vs_np_analysis.md"
    with open(report_file, 'w') as f:
        f.write(f"# Validation Log: p_vs_np_analysis\n\n")
        f.write(f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Paper:** p_vs_np_analysis\n")
