...
ORGANIZATION = {
    "archives": {
        ...
        "keywords": ["backup", "archive", "old", "deprecated"],
        "exclude": []
    },
    "tools": {
        "name": "tools",
    ...
    "data": {
        ...
        "keywords": ["data", "output", "result", "json", "csv", "database"],
        "exclude": ["__pycache__"]
    },
    "config": {
        "name": "config",
...
def categorize_file(filepath: Path) -> Tuple[str, bool]:
    ...
    # Check archives first (before other categories)
    if "archives" in ORGANIZATION:
        archive_config = ORGANIZATION["archives"]
        if archive_config["patterns"] and matches_pattern(filepath, archive_config["patterns"]):
            return "archives", False
        if archive_config["keywords"] and matches_keywords(filepath, archive_config["keywords"]):
            return "archives", False
    
    # Check each category (excluding archives which we already checked)
...
def organize_files(dry_run: bool = True) -> Dict[str, List[Path]]:
    ...
    # Ensure archives is included
    if "archives" not in organized:
        organized["archives"] = []
...
1. **Archives** - Backup files and old versions
2. **Tools** - All Python scripts and utilities
3. **Documentation** - All markdown docs and reports
4. **Research** - Research papers and analysis
5. **Benchmarks** - Benchmark results and testing
6. **Data** - Data files, outputs, and results
7. **Config** - Configuration files
8. **Projects** - Major project directories
9. **Misc** - Files that don't fit other categories
